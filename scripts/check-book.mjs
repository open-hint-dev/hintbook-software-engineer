...
if (Fs.existsSync(se) && !alignedReferences.has(manifest.id)) {
    ...
    for (const file of files.filter((name) => !name.startsWith('__'))) {
        ...
        if (seNames.has(name) && !allowed.has(name)) problems.push(`${name}: shadows hintbook-software-engineer outside the common/compatible set`);
    }
}
...
if (Fs.existsSync(Path.join(ROOT, 'testdata/example/hint.yml'))) {
    ...
    if (Fs.existsSync(Path.join(ROOT, 'emit'))) {
        const emit = spawnSync('npx', ['-y','@openhint/cli@1.5.1','emit','--check','artifact.md'], {cwd:Path.join(ROOT,'testdata/example'), encoding:'utf8'});
