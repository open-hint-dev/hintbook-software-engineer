...
- If the code already satisfies the specification, change nothing. An empty diff is the correct, expected result when the code already conforms — report it as already conformant instead of manufacturing edits. Running this mode again on the same specification and code must leave the code identical.
